...
class ReviewerController extends Controller {
    public function Invitations() {
        ...
        return view('conference.reviewer.invitations', ['conference' => $conference, 'invitations' => $invitations]);
    }
    ...
    public function AcceptInvitationPage(Request $request) {
        $reviewer = Auth::user();
        $invitation = Invitation::find($request -> id);
        if ($invitation -> reviewer -> id == $reviewer -> id && $invitation -> review == '' && $invitation -> status == 0) return view('conference.reviewer.upload_review', ['invitation' => $invitation]);
    ...
    public function UploadReview(Request $request) {
        ...
        if ($deadline < date('Y-m-d')) return back() -> with('warn', "The invitation is expired!");
            ...
            if ($request -> hasFile('file')) {
                ...
                Mail::send('email.upload_review', $data, function($message) use ($to) {
                    $message -> to($to['email'], $to['name']) -> subject('Online Submission System - Notification about Receiving Review');
                });
                return view('conference.reviewer.upload_success');
    ...
    public function RejectInvitation(Request $request) {
        ...
        Mail::send('email.reject_invitation', $data, function($message) use ($to) {
            $message -> to($to['email'], $to['name']) -> subject('Online Submission System - Notification about Rejecting Invitation');
        });
    ...
    public function ReInvitations() {
        ...
        return view('conference.reviewer.re_invitations', ['conference' => $conference, 're_invitations' => $re_invitations]);
    }
    ...
    public function AcceptReInvitationPage(Request $request) {
        $reviewer = Auth::user();
        $invitation = ReInvitation::find($request -> id);
        if ($invitation -> reviewer -> id == $reviewer -> id && $invitation -> re_review == '' && $invitation -> status == 0) return view('conference.reviewer.upload_re_review', ['invitation' => $invitation]);
    ...
    public function UploadReReview(Request $request) {
        ...
        if ($deadline < date('Y-m-d')) return back() -> with('warn', "The invitation is expired!");
            ...
            if ($request -> hasFile('file')) {
                ...
                Mail::send('email.upload_revision_review', $data, function($message) use ($to) {
                    $message -> to($to['email'], $to['name']) -> subject('Online Submission System - Notification about Receiving Revision Review');
                });
                return view('conference.reviewer.upload_success');
            } else return back() -> with('danger', "You haven't selected any file to upload!");
        }
    ...
    public function RejectReInvitation(Request $request) {
        ...
        Mail::send('email.reject_revision_invitation', $data, function($message) use ($to) {
            $message -> to($to['email'], $to['name']) -> subject('Online Submission System - Notification about Rejecting Revision Invitation');
        });
    ...
    public function Profile() {
        return view('conference.reviewer.profile');
    }
